			...
			<tbody>
				<c:forEach var="board" items="${list}">
					<tr>
						<td>${board.id}</td>
						<td><a
							href="/notice/noticeView/${board.id}${criteria.getQuery(criteria.page)}">${board.title}</a>
						</td>
						<td class="t-c">-</td>
			...
			<ul class="pagination justify-content-center align-items-center">
				<c:if test="${criteria.prev}">
					<li class="page-item"><a
						href="/notice/noticeView/${criteria.getQuery(criteria.start - 1)}"
				...
				<c:forEach var="i" begin="${criteria.start}" end="${criteria.end}"
					step="1">
					<li class="page-item"><a
						href="/notice/noticeView/${criteria.getQuery(i)}" class="page-link">${i}</a>
					</li>
				</c:forEach>

				<c:if test="${criteria.next}">
					<li class="page-item"><a
						href="/notice/noticeView/${criteria.getQuery(criteria.end + 1)}"
