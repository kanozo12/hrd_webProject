            ...
            <div class="col-10 offset-1">
                ...
                <table class="table table-striped">
                    ...
                    <c:forEach var="board" items="${list}">
                        <tr>
                            <td>${board.id}</td>
                            <td><a href="/board/view/${board.id}${criteria.getQuery(criteria.page)}">${board.title}</a>
                            </td>
                            <td>${board.writer}</td>
        ...
        <nav>
            <ul class="pagination justify-content-center">
                <c:if test="${criteria.prev}">
                    <li class="page-item">
                        <a href="/board/list${criteria.getQuery(criteria.start - 1)}" class="page-link"
                            aria-label="Previous">
                ...
                <c:forEach var="i" begin="${criteria.start}" end="${criteria.end}" step="1">
                    <li class="page-item">
                        <a href="/board/list${criteria.getQuery(i)}" class="page-link">${i}</a>
                    </li>
                </c:forEach>

                <c:if test="${criteria.next}">
                    <li class="page-item">
                        <a href="/board/list${criteria.getQuery(criteria.end + 1)}" class="page-link" aria-label="Next">
